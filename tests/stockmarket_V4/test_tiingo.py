...
def test_daily_reader_requests_each_symbol_individually() -> None:
    ...
    assert call["params"]["startDate"] == "2020-01-01"
    assert call["headers"]["Authorization"] == "Token token"


def test_daily_reader_uses_batch_endpoint(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(TiingoDailyReader, "_should_use_batch", lambda self: True)
    session = _FakeSession(
        [
            [
                {
                    "ticker": "AAPL",
                    "priceData": [
                        _payload("2020-01-01", close=1.0, volume=10.0),
                    ],
                },
                {
                    "ticker": "MSFT",
                    "priceData": [
                        _payload("2020-01-01", close=5.0, volume=20.0),
                    ],
                },
            ]
        ]
    ...
    assert df.loc[("MSFT", pd.Timestamp("2020-01-01")), "volume"] == 20.0
    assert len(session.calls) == 1
    call = session.calls[0]
...
def test_batch_request_missing_symbol_raises(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(TiingoDailyReader, "_should_use_batch", lambda self: True)
    session = _FakeSession(
        [
            [
...
def test_intraday_reader_includes_frequency() -> None:
    session = _FakeSession(
        [
            {
                "json": [
                    _payload("2020-01-01T14:30:00Z", close=1.0, volume=10.0),
                ]
            }
        ]
    )
    ...
    assert df.loc[("AAPL", pd.Timestamp("2020-01-01T14:30:00Z")), "volume"] == 10.0
    call = session.calls[0]
